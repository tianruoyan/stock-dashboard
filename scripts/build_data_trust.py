...
CORE_FILES = [
    {"file": "data/alert.json", "label": "盘中异动", "depends_on_source": True, "role": "高频触发信号"},
    {"file": "data/intraday.json", "label": "盘中全景", "depends_on_source": True, "role": "盘面结构和情绪"},
    {"file": "data/premarket.json", "label": "早盘盘前", "depends_on_source": True, "role": "开盘前研判"},
    {"file": "data/midday.json", "label": "午盘盘前", "depends_on_source": False, "role": "午后验证框架"},
    {"file": "data/postmarket.json", "label": "盘后复盘", "depends_on_source": True, "role": "收盘复盘和次日观察"},
    {"file": "data/evening-sentiment.json", "label": "晚间舆情", "depends_on_source": False, "role": "隔夜事件和公告"},
    {"file": "data/topics.json", "label": "专题跟踪", "depends_on_source": False, "role": "中期专题结论"},
    {"file": "data/decision-feed.json", "label": "机会风险流", "depends_on_source": False, "role": "结构化机会/风险/验证"},
]
...
def main() -> int:
    ...
    files = [
        trust_row(spec, payloads.get(spec["file"]), current_date, quality_issues, source_flags)
...
def trust_row(spec: dict[str, Any], data: Any, current_date: str, quality_issues: list[Any], source_flags: list[str]) -> dict[str, Any]:
    rel = spec["file"]
    path = ROOT / rel
    reasons: list[str] = []
    status = "trusted"
    ...
    return {
        ...
        "timestamp": ts or "",
        "status": status,
        "trust_score": trust_score(status, reasons),
...
def summarize(files: list[dict[str, Any]]) -> str:
    ...
    if blocked:
        ...
        return f"{len(blocked)} 个数据文件不可用（{names}），{len(degraded)} 个文件需降权。"
    if degraded or stale:
        return f"{len(degraded)} 个数据文件需降权，{len(stale)} 个文件仅作历史背景。"
...
def trust_score(status: str, reasons: list[str]) -> int:
    base = {
        "trusted": 92,
        "degraded": 62,
        "stale": 45,
    ...
    return max(0, min(100, base - min(20, len(reasons) * 4)))
