  ...
  buildFromIndex(indexHtml) {
    ...
    while ((sectionMatch = sectionRe.exec(indexHtml))) {
      ...
      for (const id of childIds) {
        ...
        child.id = id;
        if (child.parentNode && child.parentNode !== section) {
...
function checkDataRenderCoverage(document, issues) {
  ...
  addCoverageCheck(checks, "section-intraday", themeCoverageSnippet(intraday.main_trends), "盘中全景主线未渲染");
  ...
  addCoverageCheck(checks, "section-postmarket", postmarket.sentiment_indicator?.method_version === 'v1_sentiment_five_factors_20260911' && postmarket.sentiment_indicator?.score != null ? `${Number(postmarket.sentiment_indicator.score).toFixed(1)}分` : "统计尚不足", "盘后市场情绪温度或缺失说明未渲染");
...
function themeCoverageSnippet(value) {
  const name = firstThemeName(value);
  if (/医药修复链|化学制药|创新药|CRO/.test(name)) return "创新药";
  if (/老登风格切换|券商|证券|保险|白酒|畜牧|权重/.test(name)) return "金融";
  return name.split(/[\/／]/)[0];
}
